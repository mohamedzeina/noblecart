...
// Preserve scroll position across sort/filter navigations
const _savedScroll = sessionStorage.getItem('_scrollY');
if (_savedScroll) {
  sessionStorage.removeItem('_scrollY');
  window.scrollTo({ top: parseInt(_savedScroll, 10), behavior: 'instant' });
}
document.querySelectorAll('.sort-chip, .pagination a').forEach(el => {
